...
- Tests: `e2e/src/organization-settings.test.ts` (10 tests, HTTP only) covers defaults, Member-read/Owner-write, branded rendering across every hosted page (including the verification landing page) and the Account Center, the trust-fabric boundary (unknown top-level and nested keys, mixed batches), audit attribution, password-policy refusal at sign-up and password change (and the absence of a reservation behind a refusal), idle expiry with activity refresh, and that introspection is a liveness check rather than activity. Existing page-shape assertions were loosened from `toEqual` to `toMatchObject` for the added `branding` field. Full suite: 188 tests across 19 files.
- Review round applied: `OrganizationSettingsService` now derives the update allowlist and storage keys from one `ORG_SECTIONS` table and reads only the row a caller needs (the idle window is on the hot path of every Session resolution); reset completion hashes before the policy gate so a weak password and a dead link do the same work; `resolveById` takes an explicit `touch` option so token grants refresh the idle window but introspection does not; and the hosted verification result page carries branding too.
