...
- Deferred: password change and its other-Sessions cascade (ticket 14); Administrator visibility into Sessions (ticket 12); per-Organization session policy (ticket 18). The spec's "linked from Applications" is a client-UI concern; with the UI workspace removed (815bf37) this release serves the Account Center as page-data JSON, the same documented deviation as the hosted sign-in page.
