...
- Tests: `e2e/src/verified-email-change.test.ts` (9 tests, HTTP + captured email only) covers the auth gate, delivery to the new address with the old handle still authenticating, effect only on proof, single-use links, uniform refusal of a verified address and of an inert Unverified Reservation, audit events, token expiry leaving the Identity untouched, and that anonymizing one Identity destroys its address in another Identity's email-change trail. Full suite: 197 tests across 20 files.
- Review round applied: the request carries escalating per-source/per-Identity throttling (ADR-0020), so an authenticated End User cannot probe addresses or spam mail unchecked; the race-lost refusal at completion now also reaches the mailbox (and its wording no longer claims an Identity exists for the own-handle/foreign-pending reasons); anonymization additionally deletes pending requests targeting the freed address and rewrites `newEmail`/`previousEmail` wherever that address appears in the Organization's audit trail; the test vocabulary follows the glossary (`END_USER`/`endUserCookie`).
