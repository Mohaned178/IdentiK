...
- Dashboard: `frontend/src/pages/applications.tsx` calls only `/api/applications*` through `frontend/src/api.ts` (no dashboard-only back door). It shows the Client ID, per-secret label/status/timestamps, a one-time secret reveal, and Owner-only generate/revoke controls.
